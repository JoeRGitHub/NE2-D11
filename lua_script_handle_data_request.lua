...
function handle_data_request()
  -- log start
  local lf = io.open("/tmp/lua_script_status.log", "a")
  if lf then
    lf:write(os.date("!%Y-%m-%dT%H:%M:%SZ") .. " : SOLAR handle_data_request() started\n")
    lf:close()
  end
  ...
  if not ok then
    ...
    if f then
      f:write(os.date("!%Y-%m-%dT%H:%M:%SZ") .. " : SOLAR connect failed: " .. tostring(err) .. "\n")
      f:close()
    end
    return data
  end
  ...
  p = rd(s, 0x300E, 1)
  if p and #p > 4 then
    -- FIXED: use %.2fA (float) instead of invalid %.2A
  ...
  -- log finish
  local lf2 = io.open("/tmp/lua_script_status.log", "a")
  if lf2 then
    lf2:write(os.date("!%Y-%m-%dT%H:%M:%SZ") .. " : SOLAR handle_data_request() finished\n")
    lf2:close()
  end
